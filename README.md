...
标签页使用 `#overview`、`#institution`、`#playground`、`#evaluations`、`#privacy` 地址，支持刷新和浏览器前进后退。评估选择保存在 `#evaluations?run=...`，失效或无权限的实验会显示明确提示。Playground 对话在本次登录内切换标签时保留，退出后清除。

2026-09-10 浏览器回归：验证两种身份及三个示例机构登录、全部导航标签和概览快捷入口、上传弹窗/合成 JSONL 上传与删除、模板与评估下载、真实 2-step 微调、评估选择/参考答案、共享/撤回、基础及共享模型真实推理、生成期间切换标签、动画暂停/继续、研究外链。额外在 390px 宽度验证移动端导航与退出。评估下载接口另有权限、撤回与脱敏回归测试。
